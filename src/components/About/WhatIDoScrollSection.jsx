...
import TextScroller from "../ui/Animation/TextScroller";
...
function WhatIDoScrollSection() {
  ...
  const SIZE = typeof window !== 'undefined' && window.innerWidth > 1280 ? 96 * 5 : window.innerWidth > 1024 ? 100 * 4 : window.innerWidth > 768 ? 75 * 5 : 76 * 4;
  const STROKE = 1.5; // px stroke width
  const RADIUS = (SIZE - STROKE) / 2;
  const CIRCUM = 2 * Math.PI * RADIUS;
  const dashLength = 15; // px, dash size
  ...
  return (
    <div
      ref={containerRef}
      className="relative w-full h-[400vh] flex items-start justify-start flex-col"
    >
        ...
        <div className="sticky h-dvh top-0 z-20 flex items-center justify-center flex-col lg:grid lg:grid-cols-3 gap-6 sm:gap-10 text-white isolate">
          ...
          <div className="flex items-center justify-end lg:items-center lg:justify-center flex-col text-center md:text-left">
            <Heading className="text-3xl md:text-4xl lg:text-5xl">
              <TextScroller
                ...
                items={WhatIDoCardInfo.map(i => (
                  <h2 className="text-3xl md:text-4xl lg:text-5xl font-poppins">{i.title}</h2>
                ))}
              />
            </Heading>
          </div>
          ...
          <div className="flex items-center justify-center w-full h-72 md:h-80 lg:h-dvh">
            <div className="size-72 md:size-80 xl:size-96 rounded-full flex items-center justify-center relative">
              <p className="absolute top-5 md:top-0 text-2xl md:text-2xl lg:text-3xl font-poppins">
                What I Do?
              </p>
              <div className="absolute bottom-2 lg:bottom-0 group bg-white size-8 xl:size-10 flex items-center justify-center rounded-full">
              ...
              {WhatIDoCardInfo.map((item, key) => {
                ...
                return (
                  <div
                    key={key}
                    className="absolute rounded-full overflow-hidden bg-transparent size-40 md:size-48 lg:size-56 xl:size-64"
                    style={{
                  ...
                  >
                    <img
                      src={item.image}
                      alt={item.title}
                      className="absolute rounded-full size-40 md:size-56 xl:size-64"
                      style={{
                        top: "50%",
                        left: "50%",
                        transform: "translate(-50%, -50%)",
          ...
          <div className="flex items-center justify-start lg:items-center lg:justify-center flex-col text-center lg:text-left max-w-md">
            <TextScroller
              activeIndex={activeIndex}
              items={WhatIDoCardInfo.map(i => (
                <p className="text-base md:text-lg px-5 font-nunito font-light text-center lg:text-left">
                  {i.description}
                </p>
              ))}
            />
